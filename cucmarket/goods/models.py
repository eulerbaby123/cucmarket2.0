...
class GoodsInfoManager(models.Manager):
    ...
    def get_id(self, id):
        return super(GoodsInfoManager, self).get_queryset().filter(id=id, isDelete=False)
    def create_good(self, title, type, picture, price, address, description, user):
        goods = self.create(title=title, type=type, picture=picture, price=price, address=address, description=description,user=user, isDelete=False)
        return goods
